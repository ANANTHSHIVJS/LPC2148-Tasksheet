...
void delayus(unsigned intmicroseconds);
void initPLL(void)
{
	PLL0CON = 0x01 ;
	PLL0CFG = 0x24 ;// Enable PLL
...
void delayms(unsigned int milliseconds)
{
	...
	while(T0TC < milliseconds);//wait until timer reaches the desired delay
...
void delayus(unsigned int microseconds)
{
	...
	T0TC = 0; //Reset Timer
	T0TCR = 0x01; //Enable timer
	while(T0TC < microseconds);//wait until timer reaches the desired delay
	T0TCR = 0x00; //Disable timer
}